...
def MixNet(x):
    ...
    #32x32x3 -> 30x30x8
    w11 = tf.Variable(tf.truncated_normal((3,3,3,8),0,0.01))
    b11 = tf.Variable(tf.truncated_normal([8],0,0.001))

    c1 = tf.nn.conv2d(x,w11, strides = [1,1,1,1], padding='VALID') + b11
    c1 = tf.nn.relu(c1)
    
    #30x30x8 -> 28x28x8
    w12 = tf.Variable(tf.truncated_normal((3,3,8,8),0,0.01))
    b12 = tf.Variable(tf.truncated_normal([8],0,0.001))
    ...
    #14x14x8 -> 12x12x16
    w21 = tf.Variable(tf.truncated_normal((3,3,8,16),0,0.01))
    ...
    #12x12x16 -> 10x10x32
    w22 = tf.Variable(tf.truncated_normal((3,3,16,32),0,0.01))
    b22 = tf.Variable(tf.truncated_normal([32],0,0.001))
    ...
    #5x5x32 -> 3x3x64
    w31 = tf.Variable(tf.truncated_normal((3,3,32,64),0,0.01))
    b31 = tf.Variable(tf.truncated_normal([64],0,0.001))

    c3 = tf.nn.conv2d(c2,w31, strides = [1,1,1,1], padding='VALID') + b31
    c3 = tf.nn.relu(c3)
    
    #3x3x64 -> 1x1x128
    w32 = tf.Variable(tf.truncated_normal((3,3,64,128),0,0.01))
    b32 = tf.Variable(tf.truncated_normal([128],0,0.001))
    ...
    #1568+800+128
    lin1 = tf.concat([flat1, flat2,flat3], 1)
    print("lin1 shape:",lin1.get_shape())

    wl1 = tf.Variable(tf.truncated_normal((1568+800+128,256),0,0.01))
